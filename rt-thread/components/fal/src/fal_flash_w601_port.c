...
{
    wm_flash_init();
    w601_onchip_flash.len = wm_flash_total();
    w601_onchip_flash.blk_size = wm_flash_blksize();
    w601_onchip_flash.addr = wm_flash_addr();     
...
{
    return w601_onchip_flash.ops.read(offset + 0x100000, buf, size);
}

static int chip_write(long offset, const uint8_t *buf, size_t size)
{
    return w601_onchip_flash.ops.write(offset + 0x100000, (void *)buf, size);
}

static int chip_erase(long offset, size_t size)
{
    return w601_onchip_flash.ops.erase(offset + 0x100000, size);
}
#endif
...
struct fal_flash_dev w601_onchip_flash = {"w60x_onchip", 0, 0, 0, {init, read, write, erase}};
struct fal_flash_dev nor_flash0 = { "norflash0", 0, 0, 0, {nor_init, read, write, erase}};
